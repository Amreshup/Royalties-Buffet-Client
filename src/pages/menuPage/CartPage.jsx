...
const CartPage = () => {
  ...
  const handleIncrease = async (item) => {
    try {
      const response = await fetch(`https://royalties-buffet-server.onrender.com/carts/${item._id}`, {
        method: "PUT",
        headers: {
    ...
    if (item.quantity > 1) {
      try {
        const response = await fetch(
          `https://royalties-buffet-server.onrender.com/carts/${item._id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDelete =   (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://royalties-buffet-server.onrender.com/carts/${item._id}`).then(response => {
          if (response) {
            refetch();
